...
import { MessageCircle, MapPin, Phone, Scale } from "lucide-react";
  ...
  return (
    <PublicLayout>
      <div className="lp-fade space-y-8">
        <div className="text-center space-y-1.5 pt-1 haq-reveal">
          <div className="flex items-center justify-center gap-2 mb-1.5 opacity-70">
            <Scale size={16} className="text-[#C9A84C]" />
          </div>
          <h1 className="lp-display text-[30px] md:text-[38px] font-semibold text-white leading-tight">Contact Us</h1>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-7 items-start">
          <div className="space-y-5 haq-reveal-l">
            <div>
              <div className="text-[10.5px] uppercase tracking-[.28em] text-[#C9A84C] font-semibold mb-2">| Contact Us</div>
              <h2 className="lp-display text-[24px] md:text-[30px] font-semibold text-white leading-tight">
                Our <span className="text-[#C9A84C]">Branch</span> Office
              </h2>
              <div className="lp-display text-[16px] text-[#C9A84C] mt-1.5">Islamabad Head Office</div>
            </div>
            ...
            <ul className="space-y-4">
              {[
                ...
                <li key={c.t} className="flex items-start gap-3.5 group transition-transform duration-300 hover:translate-x-1">
                  <div className="w-10 h-10 rounded-full grid place-items-center shrink-0 transition-all duration-300 group-hover:scale-110 group-hover:shadow-[0_0_18px_rgba(201,168,76,.5)]"
                    style={{ background: "rgba(201,168,76,.08)", border: "1px solid rgba(201,168,76,.45)", color: "#C9A84C" }}>
                    <c.Icon size={15} />
                  </div>
                  <div>
                    <div className="text-[13px] text-white font-semibold">{c.t}</div>
                    <div className="text-[11.5px] text-[#A8A39A] mt-0.5 whitespace-pre-line leading-relaxed">{c.v}</div>
                  </div>
                </li>
              ))}
            </ul>

            <div className="rounded-2xl p-4 transition-all duration-300 hover:shadow-[0_0_30px_rgba(201,168,76,.15)]"
              style={{ background: "linear-gradient(180deg, rgba(20,16,12,.85), rgba(10,9,8,.85))", border: "1px solid rgba(201,168,76,.3)" }}>
              <div className="flex items-start gap-3 mb-2.5">
                <div className="text-[12px] text-white font-semibold leading-relaxed">
                  Choose Our Lawyer According<br />Your Cases
                </div>
                <Scale size={24} className="text-[#C9A84C] ml-auto" />
              </div>
              <div className="flex flex-wrap gap-2">
                {CASE_TAGS.map(t => (
                  <span key={t} className="px-3 py-1 rounded-full text-[10.5px] text-[#E8E0D0] transition-all duration-200 hover:text-[#C9A84C] hover:scale-105"
          ...
          <div className="rounded-2xl p-5 md:p-6 haq-reveal-r"
            ...
            {sent ? (
              <div className="text-center py-8 space-y-3">
                <div className="w-12 h-12 rounded-full grid place-items-center mx-auto" style={{ background: "rgba(201,168,76,.15)", color: "#C9A84C" }}>
                  <Scale size={22} />
                </div>
                <h3 className="lp-display text-[22px] text-white font-bold">Message Received</h3>
                ...
                <button onClick={() => { setSent(null); setName(""); setEmail(""); setPhone(""); setMsg(""); }}
                  className="px-5 h-9 rounded-full text-[12px] text-[#C9A84C]" style={{ border: "1px solid #C9A84C" }}>Send Another</button>
              </div>
            ) : (
              <form onSubmit={submit} className="space-y-3.5">
                <h3 className="lp-display text-[22px] md:text-[26px] text-white font-semibold leading-tight">
                  Reach To <span className="text-[#C9A84C]">Connect</span><br />With Us
                </h3>
                <p className="text-[11.5px] text-[#A8A39A] leading-relaxed">
                ...
                <Field label="Enter Your Message">
                  <textarea value={msg} onChange={e => setMsg(e.target.value)} placeholder="Your Message" rows={3} className="contact-field" />
                </Field>
                <button type="submit" disabled={!canSubmit}
                  className="w-full h-11 rounded-full inline-flex items-center justify-center text-[12px] font-bold uppercase tracking-[.22em] text-[#0A0A0A] transition-all duration-300 hover:scale-[1.02]"
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {BRANCHES.map((b, i) => (
            <div key={b.city} className="haq-reveal" style={{ transitionDelay: `${i * 110}ms` }}>
              <div className="relative rounded-2xl overflow-hidden p-2.5 transition-all duration-500 hover:-translate-y-1.5 hover:shadow-[0_20px_60px_-15px_rgba(201,168,76,.45)]"
                style={{ background: "rgba(20,16,12,.85)", border: `1px solid ${b.active ? "#C9A84C" : "rgba(201,168,76,.25)"}` }}>
                <div className="rounded-xl overflow-hidden mb-2.5" style={{ height: 88 }}>
                ...
                <div className="px-1 pb-1">
                  ...
                  <div className="text-[10px] text-[#A8A39A] mt-0.5">{b.line}</div>
                  <a href="#" className="inline-flex items-center mt-1.5 text-[10.5px] text-[#C9A84C] font-semibold uppercase tracking-[.14em] story-link">
                    Locate Us
                  </a>
                </div>
              </div>
