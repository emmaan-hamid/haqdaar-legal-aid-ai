...
export const AvailabilityCockpit = ({
  ...
  return (
    <div className="lp-card p-3 lp-lift h-full flex flex-col">
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-start gap-2">
          <span className="mt-1.5 inline-block w-2 h-2 rounded-full" style={{ background: dot, boxShadow: `0 0 10px ${dot}` }} />
          <div>
            <div className="text-[9px] uppercase tracking-[.2em] text-[#C9A84C] font-bold">Live Status</div>
            <div className="lp-display text-[15px] font-bold text-white leading-tight mt-0.5">{title}</div>
          </div>
        </div>
        <div className="text-[9.5px] text-[#888]">just now</div>
      </div>

      <div className="rounded-lg p-1 flex gap-1 mb-2.5" style={{ background: "#0F0F0F", border: "1px solid rgba(201,168,76,.18)" }}>
        {([
          ...
          return (
            <button key={k} onClick={() => setStatus(k as Status)}
              className="flex-1 flex items-center justify-center gap-1.5 h-7 rounded-md text-[10.5px] font-semibold transition-all"
              style={{
                color: sel ? col : "#E8E0D0",
                border: `1px solid ${sel ? col : "transparent"}`,
                background: sel ? `${col}15` : "transparent",
                boxShadow: sel ? `0 0 14px ${col}55, inset 0 0 8px ${col}22` : "none",
              }}
      ...
      <div className="rounded-lg p-2.5" style={{ background: "#0F0F0F", border: "1px solid rgba(201,168,76,.2)" }}>
        <div className="flex items-start justify-between mb-1.5">
          <div>
            <div className="text-[9px] uppercase tracking-[.2em] text-[#888] font-semibold">{capacityLabel}</div>
            <div className="text-[11px] text-[#C9C4B0] mt-0.5">Handling {active} of {maxCases}</div>
          </div>
          <div className="text-right">
            <div className="lp-display text-[18px] font-bold text-[#C9A84C] leading-none">{maxCases}</div>
            <div className="text-[8px] uppercase tracking-[.18em] text-[#888] mt-0.5">Max</div>
          </div>
        </div>
        ...
        <div className="flex justify-between text-[9px] text-[#666] mt-1 px-0.5">
      ...
      <div className="flex items-center justify-between mt-2.5 gap-3">
        <div className="text-[10.5px] flex-1" style={{ color: dot }}>{helperText[status]}</div>
        <button className="lp-btn lp-btn-gold-solid" style={{ height: 28, padding: "0 14px", fontSize: 10.5 }}>Save</button>
      </div>
    </div>
  );
};